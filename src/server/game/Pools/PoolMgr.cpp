...
template WH_GAME_API bool ActivePoolData::IsActiveObject<Creature>(uint32) const;
template WH_GAME_API bool ActivePoolData::IsActiveObject<GameObject>(uint32) const;
template WH_GAME_API bool ActivePoolData::IsActiveObject<Pool>(uint32) const;
template WH_GAME_API bool ActivePoolData::IsActiveObject<Quest>(uint32) const;
...
void PoolGroup<Quest>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom)
{
    ...
    if (limit > newQuests.size() && !currentQuests.empty())
    {
        do
        {
            uint32 questId = warhead::Containers::SelectRandomContainerElement(currentQuests);
    ...
    do
    {
        uint32 questId = warhead::Containers::SelectRandomContainerElement(newQuests);
        spawns.ActivateObject<Quest>(questId, poolId);
        PoolObject tempObj(questId, 0.0f);
        Spawn1Object(&tempObj);
